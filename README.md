...
that draws a simple GUI on the screen.
...
- `html:dist`: compiles GWT sources. The compiled application can be found at `html/build/dist`: you can use any HTTP
  server to deploy it.
- `html:superDev`: compiles GWT sources and runs the application in SuperDev mode. It will be available at
  [localhost:5000](http://localhost:8080/html). Use only during development.
