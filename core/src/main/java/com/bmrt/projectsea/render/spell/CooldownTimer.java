...
public class CooldownTimer extends Table {
    ...
    private final Image cooldownDisplay;
    ...
    private TextureRegionDrawable cooldownTimer(float remainingPercentage) {
        ...
        Pixmap display = new Pixmap(width * SCALE_FACTOR, height * SCALE_FACTOR, Pixmap.Format.RGBA8888);
        Pixmap finalDisplay = new Pixmap(width, height, Pixmap.Format.RGBA8888);

        Pixmap.Blending blending = display.getBlending();
        ...
        try {
            ...
            for (int count = 0; count < segments; count++) {
                ...
                cx = cos * cx - sin * cy;
                cy = sin * temp + cos * cy;
                display.fillTriangle(width * SCALE_FACTOR / 2, height * SCALE_FACTOR / 2,
                    (int) (width * SCALE_FACTOR / 2 + pcx), (int) (height * SCALE_FACTOR / 2 + pcy),
                    (int) (width * SCALE_FACTOR / 2 + cx), (int) (height * SCALE_FACTOR / 2 + cy));
            }

            finalDisplay.drawPixmap(display, 0, 0, width * SCALE_FACTOR / 2 - width / 2,
                height * SCALE_FACTOR / 2 - height / 2, width, height);

            display.setBlending(Pixmap.Blending.None);
